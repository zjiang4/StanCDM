#' @title Stan code generation for binary LCDM
#'
#' @description
#' Function to automate Stan code geneartion for LCDMs with binary resposnes
#'
#' @param Qmatrix the Q-matrix specified for the LCDM
#' @param savepath save the .stan file to somewhere
...
StanLCDM<-function(Qmatrix,savepath="C:\\Users\\zjiang17\\Desktop",savename="LCDM"){
  ...
  sink(file=paste(paste(savepath,savename,sep='\\'),'.stan',sep=''),append=FALSE)
    cat(
    paste(c('   ',
    data.spec,parm.spec,transparm.spec,model.spec)
  ))
  sink(NULL)
}